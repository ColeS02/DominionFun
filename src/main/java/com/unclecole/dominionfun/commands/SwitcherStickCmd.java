...
public class SwitcherStickCmd implements CommandExecutor {

    private DominionFun plugin;
    private Config config;

    public SwitcherStickCmd() {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String string, String[] args) {
        ...
        if(args.length < 3) {
            TL.INVALID_ARGUMENTS.send(s, new PlaceHolder("<command>", "/switcherstick give <name> <amount> <uses>"));
            return false;
        }
        if(args[0].equals("give")) {

            if(!s.hasPermission("switcherstick.give")) {
            ...
            ItemBuilder stick = plugin.getConfigUtils().getSwitcherStick();
            ...
            NBTItem stickNBT = new NBTItem(stick);

            stickNBT.setInteger("uses", uses);
            stickNBT.applyNBT(stick);

            for(int i = 0; i < amount; i++) {
                stickNBT.setInteger("random", ThreadLocalRandom.current().nextInt(-100000, 100000));
                stickNBT.applyNBT(stick);
                player.getInventory().addItem(stick);
            }

            if(!s.equals(player)) {
                TL.GAVE_ITEM.send(s, new PlaceHolder("%player%", player.getName()), new PlaceHolder("%amount%", amount), new PlaceHolder("%item%", "Switcher Stick"));
            }

            TL.RECEIVED_ITEM.send(player, new PlaceHolder("%amount%", amount), new PlaceHolder("%item%", "Switcher Stick"));
        }
